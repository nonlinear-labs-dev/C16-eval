...
#define pKEYS_MASKREG (uint32_t *) (0x400F6080 + 2 * 4)  // masking setup     for GPIO2[0..31]
#define pKEYS         (uint8_t *)  (0x400F6180 + 2 * 4)  // masked read/write for GPIO2[0..31]
// clang-format on
...
static __attribute__((always_inline)) inline void GetHwUpperSwitches(void)
{
  currentSwitches.upper = (uint8_t) ~(*pKEYS);  // contacts are inverting
}

static __attribute__((always_inline)) inline void GetHwLowerSwitches(void)
{
  tempLower = (uint8_t) ~(*pKEYS);  // contacts are inverting. Read to temp because of interleaving
}
...
static __attribute__((always_inline)) inline void SetHwLine(uint32_t const line)
{
  *pLINE = (uint8_t) line;  // direct write via the masked gpio register, for speed.
}
...
static __attribute__((always_inline)) inline void DetectNotes(uint32_t const key, uint16_t const mask)
{
  ...
  {  // either both switches on AND key off, or both switches off AND key on ==> key event (either on or off)
    if (keyOff[key] ^= mask)
    {
      IPC_M0_KeyBuffer_WriteKeyEvent(key | (ticker - keyTime[key]));
      return;
    }
    IPC_M0_KeyBuffer_WriteKeyEvent(key | IPC_KEYBUFFER_NOTEON | (ticker - keyTime[key]));
    return;
  }
